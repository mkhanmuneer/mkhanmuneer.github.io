...
# --- CONFIGURATION ---
ENV_NAME="ds"  # Name of the Conda environment to use
THRESHOLD_KB=1000 
LOG_DIR="logs"
...
# 5b. STAGING LEAK GUARD
# _staging/ holds projects still under review — it must never reach production.
if [ -d "docs/_staging" ] || compgen -G "docs/*staging*" > /dev/null; then
    echo "❌ ERROR: _staging content was rendered into docs/. Aborting before commit."
    echo "   Offending paths:"; ls -d docs/_staging docs/*staging* 2>/dev/null
    exit 1
fi
echo "✅ Staging guard passed (no review-only projects in docs/)."

# 5c. SITEMAP GUARD
# Quarto writes docs/sitemap.xml on every render because `site-url` is set in
# _quarto.yml. Nothing after this point copies over it. This guard fails the
# deploy loudly if that ever stops happening (e.g. site-url removed) or the
# file comes out suspiciously small, so a broken sitemap is never shipped.
SITEMAP="docs/sitemap.xml"
if [ ! -s "$SITEMAP" ]; then
    echo "❌ ERROR: $SITEMAP missing or empty after render."
    echo "   Check that 'website.site-url' is still set in _quarto.yml."
    exit 1
fi
sitemap_urls=$(grep -c '<loc>' "$SITEMAP")
sitemap_expected=$(find projects -mindepth 2 -maxdepth 2 -name index.qmd | wc -l)   # one per project + index.html
if [ "$sitemap_urls" -lt "$sitemap_expected" ]; then
    echo "❌ ERROR: $SITEMAP has only $sitemap_urls URLs; expected at least $sitemap_expected (projects) + 1 (home)."
    exit 1
fi
if grep -qiE 'cv\.html|/_staging/' "$SITEMAP"; then
    echo "❌ ERROR: $SITEMAP contains an unexpected entry (cv.html or _staging)."
    grep -iE 'cv\.html|/_staging/' "$SITEMAP"
    exit 1
fi
echo "✅ Sitemap guard passed ($sitemap_urls URLs, all generated this build)."
...
if [ $? -eq 0 ] && [ -d "docs" ]; then
    ...
    # NB: sitemap.xml is generated by Quarto (site-url) — never copy a static
    # one here or it clobbers the real, complete sitemap.
    cp google*.html robots.txt docs/ 2>/dev/null || :
